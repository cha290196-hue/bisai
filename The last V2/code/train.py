...
import numpy as np
from mode import FlowerClassifier
from utils import FlowerDataset, set_seed, save_checkpoint, load_checkpoint, prepare_data_csv

# ========== 加载配置文件 ==========
config_path = 'C:/Users/25185/Desktop/The last/model/config.json'
if not os.path.exists(config_path):
    raise FileNotFoundError(f"配置文件 {config_path} 不存在，请确保文件路径正确。")
...
def main():
    ...
    if not os.path.exists('data/train.csv'):
        ...
        df = prepare_data_csv(config['data']['train_path'], 'data/train.csv')
    else:
        print('正在加载数据...')
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        if val_acc > best_val_acc:
            ...
            save_checkpoint(
                ...
                epoch=epoch,
                val_acc=val_acc,
                filepath=os.path.join(config['paths']['checkpoint_dir'], 'best_model.pth'),
                config=config
            )
    ...
    # 保存训练历史
    history_path = os.path.join(config['paths']['checkpoint_dir'], 'training_history.json')
    with open(history_path, 'w') as f:
        json.dump(history, f, indent=4)
    print(f'训练历史已保存到: {history_path}')
    
    # 保存配置文件
    config_path = os.path.join(config['paths']['checkpoint_dir'], 'config.json')
